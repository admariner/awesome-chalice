...
- [Lucidchart](https://lucid.app/) `lucid.app`
  - Lucid is the premier tool for hand-crafting AWS Solutions Architecture diagrams. They have the 2017 and 2019 AWS icon packs by default, and you can also [download the latest AWS Architecture Icons from Amazon](https://aws.amazon.com/architecture/icons/) and upload them to Lucidchart. We now have a customized version, the [Awesome AWS Icon Pack](https://github.com/chalice-dev/awesome-chalice/blob/main/Awesome_AWS_Icon_Pack_2021_SVG.zip) which contains only the `.svg` icons, weighing in at 1.5MB. This allows you to add the entire set (900+ icons) as a Library on Lucid with select all -> drag/drop.
