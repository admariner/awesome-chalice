...
[Chalice](https://aws.github.io/chalice/)<a href="https://aws.github.io/chalice/"><img src="https://aws.github.io/chalice/_static/img/chalice-logo-icon-small.png" height="20"></a> is Amazon Web Services' premier ideas-as-code solution to writing infinitely scalable Python applications that integrate with AWS Lambda. Chalice provides an extensible build process via <code>chalice generate-pipeline</code>, allowing you to orchestrate the deployment (<code>chalice deploy</code>) of any dependencies, such as infrastructure code written in the CDK, or executable artifacts. Chalice is suited for writing https APIs that wrap infrastructure on AWS, writing libraries of Ops functions (DevOps, DevSecOps, NetDevOps, NetOps, NetSecOps, CI/CD Pipeline, ChatOps, etc.), writing web applications with your favorite front-end toolkit (or AWS Amplify), and to providing the web API backend layer to cross-platform Win/Mac/Linux/Android/iOS desktop applications written in Qt for Python (PySide6). Chalice's design is based on decorators. It is written by the AWS Chalice team. Think of Chalice as your primary entry point to whatever it is that you want to do on AWS, or in the cloud. To join the community-run Slack channel, visit https://chalice.dev
...
* `current` [AWS Cloud Development Kit (CDK)](https://aws.amazon.com/cdk/) `aws.amazon.com` ![#90EE90](https://via.placeholder.com/15/90EE90/000000?text=+)
  * The AWS CDK is written in Javascript and, via JSii, is available to Python. With the <code>chalice-cdk</code> Python module you can deploy your Chalice application via the CDK. Alternately, with Chalice's <code>chalice generate-pipeline</code> build process, you can deploy your CDK infrastructure when you deploy your Chalice application. In other words, you can either use the CDK to deploy Chalice, or use Chalice to deploy the CDK. It makes sense to use Chalice to deploy the CDK, since Chalice is better suited for Ops. However, longtime CDK users may prefer to use the CDK to deploy chalice.
...
### Paid Tracing
* `paid` [sentry-python: application monitoring platform to diagnose, fix, and optimize code performance](https://github.com/getsentry/sentry-python/blob/af163ff176b2c22952443dc5ec535aed98656fc2/tests/integrations/chalice/test_chalice.py) `GitHub`
* `paid` [Lumigo Python agent for distributed tracing and performance monitoring](https://github.com/lumigo-io/python_tracer/blob/1ab55406befe494aa61384576339b49a1e681623/src/lumigo_tracer/examples/chalice.py) `GitHub`
* `paid` [IOpipe Analytics & Distributed Tracing Agent for Python](https://github.com/iopipe/iopipe-python#chalice) `GitHub`
